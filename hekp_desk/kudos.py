# ============================================================
# KUDOS - LOCAL AI VOICE ASSISTANT (v2)
# ============================================================
#
# Architecture:
#
#   Microphone
#       ↓
#   faster-whisper (STT, offline, VAD-filtered)
#       ↓
#   Kudos Assistant
#       ↓ HTTP
#   Local AI API
...
#   Response
#       ↓
#   Piper (TTS, offline, neural, natural voice)
#
# CHANGES FROM v1:
#   - Vosk -> faster-whisper (better accuracy, built-in VAD
#     so background noise / silence gets filtered out)
#   - pyttsx3 -> Piper (neural TTS, sounds human instead of
#     robotic, still 100% offline)
#   - Models are loaded ONCE at startup, not per-call
#   - Mic capture tuned for snappier cutoffs
#
# SETUP REQUIRED BEFORE RUNNING:
#   pip install faster-whisper piper-tts sounddevice soundfile numpy
#
#   Download a Piper voice (.onnx + .onnx.json), e.g.:
#   https://github.com/rhasspy/piper/blob/master/VOICES.md
#   -> en_US-amy-medium.onnx / en_US-amy-medium.onnx.json
#   Put both files next to this script (or update PIPER_VOICE_PATH).
#
#   faster-whisper will auto-download the "small.en" model
#   the first time it runs (needs internet ONCE, then it's
#   cached locally and works fully offline after that).
# ============================================================
...
# ------------------------------------------------------------
# Model size vs speed/accuracy tradeoff:
#   "tiny.en"  -> fastest, least accurate
#   "base.en"  -> good middle ground
#   "small.en" -> more accurate, still real-time on most CPUs
WHISPER_MODEL_SIZE = "small.en"
WHISPER_COMPUTE_TYPE = "int8"   # int8 = fast on CPU, low RAM

# ------------------------------------------------------------
# Text-to-speech (Piper)
# ------------------------------------------------------------
PIPER_VOICE_PATH = "/run/media/ali/New Volume/Work Space/University/FAST/Classes/Summers_2026/Project/College_environment/hekp_desk/en_US-amy-medium.onnx"


# ============================================================
# INITIALIZE MODELS (load once, not per-call)
# ============================================================

print(Fore.YELLOW + "Loading speech recognition model..." + Style.RESET_ALL)
...
print(Fore.YELLOW + "Loading text-to-speech voice..." + Style.RESET_ALL)
...
# ============================================================
# SPEECH RECOGNITION SETTINGS (mic capture)
# ============================================================
...
recognizer.pause_threshold = 0.5   # snappier cutoff than v1 (was 0.8)
...
# ============================================================
# SPEECH-TO-TEXT (faster-whisper, with built-in VAD)
# ============================================================

def recognize_speech(audio):
    """
    Convert SpeechRecognition AudioData into text locally using
    faster-whisper. vad_filter=True automatically strips out
    silence and background noise before transcribing, so it
    also improves accuracy in noisy rooms.
    """
    ...
    try:
        ...
        data, samplerate = sf.read(io.BytesIO(raw_audio))
